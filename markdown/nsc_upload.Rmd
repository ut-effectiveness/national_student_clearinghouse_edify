...
## Prior to running all code chunks make sure you are connected to /Share/Research/National Student Clearing House, add a folder with the month and year and update the folder on lines 159, 177, 196, and 215.  These files should be uploaded to the National Student Clearing House in the Fall after October 31st and in the Spring after March 15th.
...
nsc_from_edify_applicant <- utHelpR::get_data_from_sql_file('nsc_from_edify_applicant.sql','edify','project')
...
change_last_name <- function(input_df) {
  input_df %>%
    mutate(
      last_name = {
        cleaned <- str_remove_all(last_name, "[^A-Za-z ]")
        cleaned <- trimws(cleaned)

        if_else(
          is.na(cleaned) | cleaned == "" | 
            !str_detect(cleaned, "(?=.*[A-Za-z])"),
          NA_character_,
          cleaned
        )
      }
...
## Function to change first names that are "." to NA
```{r}
change_first_name <- function(input_df) {
  input_df %>%
    mutate(
      first_name = {
        cleaned <- str_remove_all(first_name, "[^A-Za-z ]")
        cleaned <- trimws(cleaned)
...
## Run files through to update last name from . to NA
```{r}
edify_enrolled_df <- change_first_name(edify_enrolled)
edify_applicant_df <- change_first_name(edify_applicant)
edify_inquiry_df <- change_first_name(edify_inquiry)
edify_prospect_df <- change_first_name(edify_prospect)
```


## Create Enrolled file for upload
```{r}
enrolled_file <- edify_enrolled_df %>% 
  mutate(
    detail_records = 'D1',
    ssn = str_pad(' ',9),
...
create_edify_file <- function(input_df) {
  ...
  output_df <- input_df %>%
    mutate(
      ...
      first_name = {
        cleaned <- str_remove_all(first_name, "[-,.*()`']")
        cleaned <- trimws(cleaned)
        cleaned <- if_else(is.na(cleaned) | cleaned == "", "UNKNOWN", cleaned)
        str_sub(str_pad(cleaned, 20, side = "right"), 1, 20)
      },

      middle_initial = replace_na(
        substr(str_remove_all(middle_initial, "\\."), 1, 1),
        ""
      ),

      last_name = {
        cleaned <- str_remove_all(last_name, "[-,.*()`']")
        cleaned <- trimws(cleaned)
        cleaned <- if_else(is.na(cleaned) | cleaned == "", "UNKNOWN", cleaned)
        str_sub(str_pad(cleaned, 20, side = "right"), 1, 20)
      },
      ...
      suffix = str_pad(
        replace_na(str_remove_all(name_suffix, "[-,.']"), ""),
        5,
        side = "right"
      ),

      birth_date = str_remove_all(birth_date, "-"),
    ...
    select(
      detail_records, ssn, first_name, middle_initial, last_name,
      suffix, birth_date, term_start_date, blank_column,
      school_code, branch_code, requester_return_field
    )
...
```{r}
applicant_file <- create_edify_file(edify_applicant_df)
inquiry_file <- create_edify_file(edify_inquiry_df)
prospect_file <- create_edify_file(edify_prospect_df)
```
